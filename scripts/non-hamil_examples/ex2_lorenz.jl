...
data = ODE.solve(
    prob, Tsit5(), abstol = 1e-12, reltol = 1e-12, saveat = trange, tstops = trange)
x = Array(data)
...
plot!(p3, xB, vars = (0, 1), linecolor = :red, linestyle = :dash,
    linewidth = 1.5, label = "approx_state1")
p4 = plot(xA, vars = (0, 2), linecolor = :black, linewidth = 1.5, label = "true_state2")
plot!(p4, xB, vars = (0, 2), linecolor = :red, linestyle = :dash,
    linewidth = 1.5, label = "approx_state2")
display(plot(p3, p4, layout = (1, 2), show = true, reuse = false,
    size = (1000, 1000), xlabel = "Time", ylabel = "X"))
...
dtA = [0; diff(xA.t)]
dtB = [0; diff(xB.t)]
p5 = plot(xA[1, :], xA[2, :], xA[3, :], zcolor = dtA, xlabel = "x",
    ylabel = "y", zlabel = "z", label = "true")
p6 = plot(xB[1, :], xB[2, :], xB[3, :], zcolor = dtB, xlabel = "x",
    ylabel = "y", zlabel = "z", label = "approx")
