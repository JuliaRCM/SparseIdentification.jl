...
method = HamiltonianSINDy(grad_H_ana, λ = 0.05, noise_level = 0.05, integrator_timeStep = 0.01, 
                            polyorder = polyorder, trignometric = trig_wave_num)
...
for i in 1:5
    ...
    scatter!(p1, data_sindy.t, data_sindy.q[:,1], label = "Identified q₁")

    p3 = plot(xlabel = "Time", ylabel = "p₁")
    scatter!(p3, data_reference.t, data_reference.q[:,3], label = "Data p₁")
    scatter!(p3, data_sindy.t, data_sindy.q[:,3], label = "Identified p₁")
    ...
    scatter!(p2, data_sindy.t, data_sindy.q[:,2], label = "Identified q₂")

    p4 = plot(xlabel = "Time", ylabel = "p₂")
    scatter!(p4, data_reference.t, data_reference.q[:,4], label = "Data p₂")
    scatter!(p4, data_sindy.t, data_sindy.q[:,4], label = "Identified p₂")
