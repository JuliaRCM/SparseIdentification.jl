...
const nparam = calculate_nparams(d, polyorder, usesine, trig_wave_num)
...
function loss(a::AbstractVector)
    ...
    for i in axes(x, 2)
        for j in 1:size(x,1)-3
            result[j:j+3,i] = hamilGradient_general!(out, x[j:j+3,i], a, 0)
        end
    end
...
for i in 1:num_trajec
    prob_approx = ODEProblem(hamilGradient_general!, x₀[:,i], tspan, a)
