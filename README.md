...
- **`using <Package>`**, which catches a syntax error or a broken `include` — **blocks**. Note that
  this package does not currently load, because `PreallocationTools` moved its `ForwardDiff`
  methods into an extension, so this check will block a commit that stages a `.jl` file until that
  is resolved.
